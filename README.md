...
<p align="center">
  <img src="doc/architecture/standard_processing_model.png?raw=true">
  <br>
  Taken from the <a href="http://www.e-geos.it/products/pdf/csk-product%20handbook.pdf">COSMO-Skymed Products Handbook<a>
</p>
...
The processes administration is done trough the _Configuration_ component. Its configuration interface is available on the _{host}/configuration_ URI, trough HTTP.

#### Uploading a Process

Using the _{host}/configuration/processes_ URI with the _POST_ HTTP method, you'll be able to publish a process. To do so, you have to send two things to the server, using the _multipart/form-data_ HTTP header:
 * The process metadata, in the key specified by the _PROCESS_SPECIFICATION_FIELD_ configuration variable of the [Configuration component settings](/lycheepy/configuration/configuration/settings.py), which by default is "specification".
 * The process file, in the key specified by the _PROCESS_FILE_FIELD_ of the [Configuration component settings](/lycheepy/configuration/configuration/settings.py), which by default is "file".
 
The following is an example of the [L0](/tests/cosmo/processes/L0.py) processor metadata:
```json
{
  "identifier": "L0",
  "title": "L0 Processor",
